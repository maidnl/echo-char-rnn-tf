...
# Description: Implementation of a Vanilla RNN in the form tanh(xW + s_t-1 U + b)
...
class RightMulModel:
    ...
    # ------------------------------------------------------------------------------
    def __init__(self, num_of_classes = 3, train_seq_len = 15, state_size = 5, batch_size = 3):
    ...
    def define_variables(self):
        ...
        # s_t = tanh(x * U + s_t-1 * V + b)
        self.U    = tf.Variable(np.random.rand(self.num_of_classes, self.state_size), dtype=tf.float32, name = "input2state")
        self.W    = tf.Variable(np.random.rand(self.state_size, self.state_size), dtype=tf.float32, name = "state2state")
        self.bias = tf.Variable(np.zeros((1,self.state_size)), dtype=tf.float32, name = "bias_state")
        # y = s_t * V + b2
        self.V     = tf.Variable(np.random.rand(self.state_size,self.num_of_classes),dtype=tf.float32, name = "state2output")
        self.bias2 = tf.Variable(np.zeros((1,self.num_of_classes)), dtype=tf.float32, name = "bias_output")
    ...
    def define_learning_placeholders(self):
        # input
        self.x_placeholder = tf.placeholder(tf.float32, [self.batch_size, self.num_of_classes * self.train_seq_len], name="inputs")
        # output (used only during learning)
        self.y_placeholder = tf.placeholder(tf.float32, [self.batch_size, self.num_of_classes * self.train_seq_len], name="targets")
        # state
        self.state         = tf.placeholder(tf.float32, [self.batch_size, self.state_size])
    ...
    def model_learn(self, inputs_series, output_series):
        ...
        for i in range(self.train_seq_len):
            ...
            start   = i * self.num_of_classes
            stop    = start + self.num_of_classes

            next_state = tf.tanh(tf.matmul(inputs_series[:,start:stop],self.U) + tf.matmul(self.current_state, self.W) +  self.bias)  
            self.current_state = next_state
            # calculate logits
            logit = tf.matmul(self.current_state,self.V) + self.bias2
            self.logits_series.append(logit)
            # calculate predictions
            prediction = tf.nn.softmax(logit)
            self.predictions_series.append(prediction)

            label = output_series[:,start:stop]
        ...
        with tf.Session(graph=self.learnin_graph) as sess:
            ...
            for epoch_id in range(self.num_epochs):
                ...
                for batch_id in range(num_batches):
                    ...
                    # HOT ENCODE input and output
                    # here the x_batch and the y_batch are a matrix which has a number of 
                    # of rows equal to the batch dimension and a number of columns equal
                    # to the training sequence (each element of the matrix is an 
                    # integer )

                    # the following performs hot encoding: each integer is translated
                    # into the correspondent hot encoding array 
                    # this means that the number of column is multiplied for the dimension
                    # of the encoding so that at end each integer is substitued with
                    # ist corresponding hot encoded version

                    # for example 1,2,1 with num_of_classes equal to 3 is exploded in
                    # 0 1 0, 0 0 1, 0 1 0

                    # the number of row remain unchanged

                    first_chunck = True
                    for i in range(self.train_seq_len):
                        xh = np.zeros((self.batch_size,self.num_of_classes))
                        yh = np.zeros((self.batch_size,self.num_of_classes))

                        xh[np.arange(self.batch_size),x_batch[:,i]] = 1 
                        yh[np.arange(self.batch_size),y_batch[:,i]] = 1
                        ...
                        if first_chunck:
                            ...
                            x_bath_hot_enc = np.concatenate((x_bath_hot_enc,xh),axis = 1)
                            y_bath_hot_enc = np.concatenate((y_bath_hot_enc,yh),axis = 1)  
                    ...
                    if(total_loss < min_loss):
                        min_loss = total_loss
                        save_path = self.saver.save(sess, "./models/RightMulModel.ckpt")
    ...
    def define_placeholders(self):
        # no need for y
        self.x_placeholder = tf.placeholder(tf.float32, [self.batch_size, self.num_of_classes], name="inputs")
    ...
    def model(self, inp):
        ...
        self.current_state = self.state 
        next_state = tf.tanh(tf.matmul(inp,self.U) + tf.matmul(self.current_state,self.W) +  self.bias)  
        self.current_state = next_state
        self.logit = tf.matmul(self.current_state,self.V) + self.bias2
    ...
    def execute(self,inp):            
        ...
        with tf.Session(graph=self.model_graph) as sess:
            ...
            if os.path.exists("./models/RightMulModel.ckpt.index") and os.path.exists("./models/RightMulModel.ckpt.meta"): 
                # restore the variable learned
                self.saver.restore(sess, "./models/RightMulModel.ckpt")
                print("Model restored from file.")
            else:
                print("\n\nModel is not available or it has not been trained yet!")
                return None
            # init the state for the first time
            current_state = np.zeros((1, self.state_size))
            # for all inputs execute the model
            for i in range(len(inp)):
                # hot encoding of input
                inp_hot_enc = np.zeros((1,self.num_of_classes))
                inp_hot_enc[0,inp[i]] = 1
...
if __name__ == "__main__":
    if(len(sys.argv) != 2):
        ...
        # LEARN
        if(sys.argv[1] == "LEARN"):
            model = RightMulModel(num_of_classes = NUM_OF_CLASSES, train_seq_len = MAX_TRAIN_SEQUENCE, state_size = STATE_DIM, batch_size = BATCH_SIZE)
            ...
            x,_           = reshape_for_batch(x, batch_dim = BATCH_SIZE, batch_len = model.train_seq_len)
            y,num_batches = reshape_for_batch(y, batch_dim = BATCH_SIZE, batch_len = model.train_seq_len)
        ...
        # execute the model 
            ...
            model = RightMulModel(num_of_classes = NUM_OF_CLASSES, train_seq_len = 1, state_size = STATE_DIM, batch_size = 1)
